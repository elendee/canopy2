...
const track_look = e => { // ( right click )
	...
	if( !PLAYER.box ) return // pre model-load

	diffX = e.clientX - lastX
	diffY = e.clientY - lastY

	// PLAYER.box.rotateX( diffY / 300 )
...
let unpan
let unpan_count = window.unpan_count = 0
const unpan_cam = () => {
...
const scroll_dist = new THREE.Vector3()
...
const camera_look_home = () => {
	...
	// if( !PLAYER.box ) return
	// console.log('ya.. runs')
	CAMERA.lookAt( new THREE.Vector3().copy( PLAYER.box.position ).add( CAMERA.fixture.position ) )
}
...
function detect_object_clicked( e, bounds ){
	...
	if( !intersects.length ){ // no more skybox woot  xx1 == skyboxx
		BROKER.publish('TARGET_SET', { 
			uuid: false, 
			mesh: false, 
			caller: 'clear'
